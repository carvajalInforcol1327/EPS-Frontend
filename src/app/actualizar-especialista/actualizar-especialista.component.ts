...
export class ActualizarEspecialistaComponent implements OnInit {
  ...
  constructor(private especialistaService:EspecialistaService,private router:Router, private route:ActivatedRoute,private especialidadService: EspecialidadService){}
  ...
  ngOnInit(): void {
    ...
    this.especialistaService.buscarEspecialistaId(this.cedula).subscribe(
      response => {
        ...
        const especialidadEncontrada = this.especialidades.find(e => e.especialidad_id === this.especialista.especialidad?.especialidad_id);
        if (especialidadEncontrada) {
          this.especialista.especialidad = especialidadEncontrada;
          console.log('Especialidad del especialista:', this.especialista.especialidad);
        } else {
  ...
  irListaEspecialista(){
    ...
    swal('Especialista actualizado',`El especialista ${this.especialista.nombres} fue actualizado con exito`,`success`);
  }

  onSubmit(){
    this.especialistaService.actualizarEspecialista(this.cedula,this.especialista).subscribe(response => {
